...
def rejection_sample(sigma, N_samples, N_candidates):
    ...
    while len(R_samples) < N_samples:
        ...
        den = 2*np.pi*sigma**2 * multivariate_normal.pdf(R_candidates, mu, cov)
...
R_samples, _ = rejection_sample(sigma, N_samples, N_candidates)
...
#     Z = np.sum(f * dr1 * dr2)
#     M = 2 * np.pi * si**2 * np.exp(si**2 / 4) / Z
#     M_array.append(M)
